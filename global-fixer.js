...
(function () {
  ...
  // =====================================================================
  // F6.1 / F6.2 — Boton WhatsApp en Contact
  // =====================================================================
  // El template ya tiene un boton flotante .div-block-108 > a con
  // href='#' roto. Lo arreglamos con el numero real.
  // Ademas inyectamos un boton inline encima del formulario de contacto.
  function fixWhatsAppContact() {
    // 1) Arreglar el boton flotante existente del template (todas las paginas)
    ...
    // 2) Boton inline encima del formulario (solo en /contact-us)
    if (!/contact-us/.test(location.href)) return;
    if (document.querySelector('.maquinasa-whatsapp-btn')) return;

    // Anchor: usar .form-get-in para insertar como primer hijo
    var formGetIn = document.querySelector('.form-get-in');
    var anchor = formGetIn ||
                 document.querySelector('.contacts-form-wrapper') ||
                 document.querySelector('#email-form');
    if (!anchor) return;

    var btnHTML = [
      '<a href="https://wa.me/34637038528" target="_blank" rel="noopener" class="maquinasa-whatsapp-btn">',
    ...
    // Insertamos como primer hijo del contenedor del formulario
    anchor.insertAdjacentHTML('afterbegin', btnHTML);

    injectCSS('maquinasa-whatsapp-css', [
      '.maquinasa-whatsapp-btn {',
      '  display: flex !important;',
      '  align-items: center;',
      '  justify-content: center;',
      '  gap: 10px;',
      ...
      '  z-index: 10;',
      '}',
      '.maquinasa-whatsapp-btn:hover {',
      '  transform: translateY(-2px);',
      '  box-shadow: 0 6px 20px rgba(37,211,102,.5);',
      '  color: #ffffff !important;',
      '}',
      '.maquinasa-whatsapp-btn svg {',
      '  flex-shrink: 0;',
      '}'
    ].join('\n'));
    log('F6.1 WhatsApp button inyectado encima del form');
  }
